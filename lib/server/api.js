...
function getBallValueAsInt(ball) {
  if (ball === undefined) {
    return 0;
  }
...
function getSanitizedFrames(frames) {
  ...
  for (var i = 0; i < 10; i++) {
    ...
    var ballThree = 0;
    if (frame !== undefined) {
      ballOne = getBallValueAsInt(frame.ballOne);
      ballTwo = getBallValueAsInt(frame.ballTwo);
      ballThree = getBallValueAsInt(frame.ballThree);
    }
